...
async function main() {
  ...
  // ── SCHEDULES & SESSIONS (for enrollment flows) ─────────────────────────
  ...
  // ── enrollment (Bookings + Trial + Workshop) ───────────────────────────
  ...
  const trial1 = await prisma.trialBooking.upsert({
    where: {
      studentId_subClassId: { studentId: student1.id, subClassId: subYoga.id },
    },
    update: { status: 'CONFIRMED' },
    create: {
      studentId: student1.id,
  ...
  const workshopBooking = await prisma.workshopBooking.upsert({
    where: {
      workshopId_studentId: { workshopId: workshop.id, studentId: student2.id },
    },
    update: { status: 'CONFIRMED' },
    create: {
  ...
  await prisma.notification.upsert({
    ...
    create: {
      ...
      body: 'Your Hip-Hop session enrollment is confirmed (alt seed).',
      sentAt: new Date(),
      bookingId: booking1.id,
    },
  });
  ...
  await prisma.galleryItemPerson.createMany({
    data: [
      { galleryItemId: galleryItem.id, galleryPersonId: galleryPerson.id },
    ],
    skipDuplicates: true,
  });
  ...
  console.log(
    [
      ...
      '- enrollment flows: schedules, sessions, bookings, trial, workshop',
