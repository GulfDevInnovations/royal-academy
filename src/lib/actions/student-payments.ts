...
export async function getStudentPayments(): Promise<{
  ...
  try {
    ...
    if (!dbUser?.studentProfile) return { data: [], error: "Student profile not found" };

    const studentId = dbUser.studentProfile.id;

    const [monthlyEnrollments, multiMonthEnrollments, trialBookings, workshopBookings, bookings] =
      await Promise.all([
...
for (const e of monthlyEnrollments) {
  ...
  const dayString =
    schedules.length > 0
      ? schedules.map((s) => s.dayOfWeek.charAt(0) + s.dayOfWeek.slice(1).toLowerCase()).join(" & ")
      : "—";
  ...
  records.push({
    ...
    frequency:    e.frequency === "ONCE_PER_WEEK" ? "Once per week" : "Twice per week",
...
for (const e of multiMonthEnrollments) {
  ...
  const startLabel = new Date(e.startYear, e.startMonth - 1)
    .toLocaleString("en", { month: "short", year: "numeric" });
  const endLabel = new Date(e.endYear, e.endMonth - 1)
    .toLocaleString("en", { month: "short", year: "numeric" });
  ...
  records.push({
    ...
    for (const t of trialBookings) {
      ...
      records.push({
        ...
        dayOfWeek:    schedule.dayOfWeek.charAt(0) + schedule.dayOfWeek.slice(1).toLowerCase(),
