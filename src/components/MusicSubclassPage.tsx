...
export default function MusicSubclassPage({
  locale,
  title,
  description,
  highlights,
  ...
  const enrollmentHref = (() => {
    ...
    params.set('dept', 'music');
    if (enrollmentQuery?.q) params.set('q', enrollmentQuery.q);
    const queryString = params.toString();
    return queryString ? `/${locale}/enrollment?${queryString}` : `/${locale}/enrollment`;
  })();
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3 md:items-stretch">
          <div className="liquid-glass h-full rounded-2xl border border-royal-cream/10 p-6">
            ...
            <p className="mt-3 text-sm leading-7 text-royal-cream/85 sm:text-[15px]">
              {isArabic
                ? 'تعليم موسيقي متخصص يطوّر المهارة والإحساس الفني — مع مسار مناسب لكل مستوى.'
                : 'Specialised music instruction that develops skill and artistry — with a path for every level.'}
            </p>
          </div>
