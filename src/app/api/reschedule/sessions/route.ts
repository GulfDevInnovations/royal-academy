...
export async function GET(req: NextRequest) {
  ...
  const enrollmentType = searchParams.get('enrollmentType') as
    | 'SINGLE'
    | 'MULTI'
    | null;
  ...
  if (!enrollmentId || !enrollmentType)
    return NextResponse.json(
      { error: 'enrollmentId and enrollmentType are required' },
      { status: 400 },
    );
  ...
  if (enrollmentType === 'SINGLE') {
    ...
    scheduleIds = parseJsonArray<string>(enrollment.scheduleIds);
    ...
    if (scheduleIds.length === 0) {
      ...
      const rows = await prisma.classSchedule.findMany({
        where: {
          subClassId,
          status: 'ACTIVE',
          dayOfWeek: { in: uniqueDays as any[] },
        },
        select: { id: true },
      });
    ...
    let m = enrollment.startMonth,
      y = enrollment.startYear;
    while (
      y < enrollment.endYear ||
      (y === enrollment.endYear && m <= enrollment.endMonth)
    ) {
  ...
  if (scheduleIds.length === 0)
    return NextResponse.json({
      sessions: [],
      debug: { reason: 'no scheduleIds resolved', subClassId },
    });
  ...
  const enrollmentStart = new Date(
    Math.min(
      ...allowedMonths.map((m) => new Date(m.year, m.month - 1, 1).getTime()),
    ),
  );
  ...
  console.log(
    '[reschedule/sessions] sessionDatetime query count:',
    sessions.length,
  );
  ...
  if (sessions.length === 0) {
    ...
    console.log(
      '[reschedule/sessions] sessionDate fallback count:',
      sessions.length,
    );
  }
  ...
  const result = sessions
    ...
    .filter(
      (s) =>
        s.bookingStatus !== 'CANCELLED' && s.bookingStatus !== 'RESCHEDULED',
    );
