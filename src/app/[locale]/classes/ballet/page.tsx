...
const shiftSeconds = (value: string, delta: number) =>
  `${(Number.parseFloat(value) + delta).toFixed(2)}s`;
...
const BALLET_GROUND_SPARKLES = GROUND_SPARKLE_OFFSETS.flatMap((offset) =>
  BASE_BALLET_SPARKLES.map((sparkle, index) => ({
    ...
    size: index % 5 === 0 ? '0.22rem' : index % 3 === 0 ? '0.18rem' : '0.14rem',
...
function LatinShortform({ children }: { children: string }) {
  return (
    <span
      lang="en"
      dir="ltr"
      className="inline-block font-sans tracking-normal"
    >
      {children}
    </span>
  );
...
export default function BalletPage() {
  ...
  const content = isArabic
    ? {
        ...
        paragraphs: [
          <>
            بصفتنا أول مدرسة باليه مسجلة في عُمان تحت إشراف الأكاديمية الملكية
            للرقص في المملكة المتحدة (<LatinShortform>RAD</LatinShortform>)، يتم
            تقديم برنامج الباليه لدينا بما يتماشى مع منهج{' '}
            <LatinShortform>RAD</LatinShortform> المعترف به دوليًا.
          </>,
          <>
            يتلقى الطلاب المسجلون في مسار <LatinShortform>RAD</LatinShortform>{' '}
            تدريبًا منظمًا ومتدرجًا، ويخضعون لامتحانات رسمية سنوية، ويحصلون على
              ...
              <div className="relative flex items-end justify-end">
                {isRadBalletOpen ? (
                  ...
                  >
                    ...
                    <p
                      className={`mt-3 text-[0.92rem] leading-6 text-white/82 ${isArabic ? 'text-right' : ''}`}
                    >
